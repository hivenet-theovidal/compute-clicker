        ...
        return (
          <motion.button
            ...
            className={`
              ...
              ${buyable
                ? 'border-line-2 bg-surface-2/60 hover:bg-surface-3/60 hover:border-faint cursor-pointer'
                : 'border-line/50 bg-surface-2/20 cursor-not-allowed opacity-50'
              }
            `}
          >
            ...
            <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-surface-3 flex items-center justify-center">
              <Image
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                <span className="text-fg text-sm font-semibold">{def.name}</span>
                <span className="text-muted text-xs ml-2">×{owned}</span>
              </div>
              <div className="text-muted text-xs truncate">{def.description}</div>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-accent-fg text-xs font-mono">{formatEuros(cost)}</span>
                <span className="text-info-fg text-xs">+{def.baseEps * regionDef.multiplier}/s</span>
              </div>
            </div>

            {!requirementsMet && def.requires && (
              <div className="text-xs text-faint text-right flex-shrink-0">
